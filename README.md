...
A [home-manager] module for generating local [protonfixes].
...
## Usage (Flakes)
 ...
 1. Add `github:eth-p/nix-protonfixes` to the flake inputs.

    ```nix
    inputs = {
      url = "github:eth-p/nix-protonfixes";
    };
    ```

 2. Add `nix-protonfixes` to the `output` function arguments.

    ```nix
    outputs = { nix-protonfixes, ... }: {
      # ...
    }
    ```
 ...
 3. Add the home-manager module to your `homeManagerConfiguration`.
    ...
    homeConfigurations.my-user = home-manager.lib.homeManagerConfiguration {
      modules = [
        nix-protonfixes.homeManagerModules.default
        # ...
      ];
    }
    ```
