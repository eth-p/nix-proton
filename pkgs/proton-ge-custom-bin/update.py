...
def add_release(release: github.Release, version: str):
    ...
    for asset in assets:
        # x86_64-linux
        if asset.name == f"{release.tag}.tar.gz":
            add_asset(system="x86_64-linux", manifest=manifest_reg)
            continue

        # aarch64-linux
        if asset.name == f"{release.tag}-aarch64.tar.gz":
        ...
        if asset.name.endswith(".tar.gz"):
            ...
            raise Exception(f"New proton-ge-custom package: {asset.name}")
...
def run():
    for release in github.get_releases(repo):
        ...
        manifest_reg.version.setup(version, package=package_name)
